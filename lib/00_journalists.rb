...
nb_of_handles = journalists.length
puts "This list has #{nb_of_handles} handles"

#2 Quelle est le handle le plus court de cette liste ?
    # Order the array by lenght
    # Take the first element
journalists.sort_by(&:length)
ordered_arr_by_lenght = journalists.sort_by(&:length)
puts "The sortest element in the array is #{ordered_arr_by_lenght[0]}"


#3 Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
names_with_5_ch = journalists.count {|x| x.length == 6}
puts "In the array journalists there are #{names_with_5_ch} elements with 5 characters"

#4 Combien commencent par une majuscule (première lettre juste après le @) ?
names_first_maj = journalists.count {|x| x[1].match?(/[A-Z]/)}
puts "In the array journalists there are #{names_first_maj} elements that starts with a capital letter"
...
puts "The index of the user @epenser is #{index_epenser}"
...
#8 Sors-moi une répartition des handle par taille de ces derniers (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)
    # I get the ordered list by elements' lenght from exercise 2 and 6
    # I get the max number of characters from the last element of the array
    # I iterate the array to count from 1 until max characters
        # in every iteration I count the elements with the same number of characters from exercise 3


#the @ symbol doesn't count as character
max_characters = ordered_arr_by_lenght.last.length

puts "##################"
(2..max_characters).each do |n|
    result = journalists.count {|x| x.length == n}
    final_n = n-1
    puts "There are #{result} user(s) with #{final_n} characters"
end
